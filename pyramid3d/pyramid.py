...
source=[mp.Source(mp.GaussianSource(frequency=fcen,fwidth=df,cutoff=2),	#gaussian current-source
		...
		#center=mp.Vector3(0,0,sz/2-sh-pyramid_height+pyramid_height*(1/source_pos)))]
		center=mp.Vector3(0,0,0))]

source2=[mp.Source(mp.ContinuousSource(frequency=fcen,start_time=0,end_time=1),
		component=source_direction,
		center=mp.Vector3(0,0,big_offset))]
...
if far_field_calculation == 'true no':

		nearfieldregion1=mp.Near2FarRegion(					#nearfield -z. above pyramid.		
			center=mp.Vector3(0,-sz/4+2*padding,0),
		...
		nearfieldregion2=mp.Near2FarRegion(
			center=mp.Vector3(sx/4-padding*2,0,0),
		...
		nearfieldregion4=mp.Near2FarRegion(
			center=mp.Vector3(0,sy/4-padding*2,0),
...
if far_field_calculation == 'true':
		...
		nearfieldregion2=mp.Near2FarRegion(
			center=mp.Vector3(sx/2,0,-(sz-sh)/2),
			size=mp.Vector3(0,sy,sz-sh-padding),
			direction=mp.X)
		...
		nearfieldregion4=mp.Near2FarRegion(
			center=mp.Vector3(sy/2,0,-(sz-sh)/2),
			size=mp.Vector3(sx,0,sz-sh-padding),
			direction=mp.Y)
...
###FAR FIELD CALCULATIONS#######################################################
		...
		nearfield=sim.add_near2far(fcen,df,nfreq,nearfieldregion1)		#calculate fields for near field region (?)

###RUN##########################################################################
sim.run(mp.synchronized_magnetic(mp.output_poynting,mp.output_tot_pwr),until=10)
...
if far_field_calculation == 'true':
	...
	npts=800							#number of far-field points
	Px=0
	Py=0
	Pz=0
...
#		flux_tot_pf_ratio.append(flux_top_out[i]/flux_tot_out[i])	#takes the ratio of flux top/total out for each frequency
	...
	if far_field_calculation == 'true':
		...
		print('Total Flux:',flux_tot_out,'Flux farfield:',P_tot_ff,'ratio:',flux_tot_ff_ratio,'simulation_time:',simulation_time,'dpml:',dpml,'res:',resolution,'r:',r,'npts:',npts)
